...
\title{Cloud Computing Lab 1 Report}
\author{Huizhe Su, Arihant Jain, Jordy, Nizar}
\date{February 2026}
...
\makebox[\textwidth][c]{%
    \begin{minipage}{0.33\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/ec2_apt_update_upgrade.png}
        \captionof{figure}{Huize Su}
    \end{minipage}
    \begin{minipage}{0.33\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/ec2_apt_update_upgrade.png}
        \captionof{figure}{Other Student}
    \end{minipage}
    \begin{minipage}{0.33\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/ec2_apt_update_upgrade.png}
...
We explored ZeroSSL as an option for obtaining a trusted certificate. However, ZeroSSL's free tier has limitations --- it requires manual renewal every 90 days, the dashboard-based workflow is less straightforward for server-side automation, and domain verification can be cumbersome compared to other solutions. For these reasons, we opted to skip ZeroSSL and proceed directly with Let's Encrypt and Certbot, which offers a fully automated and CLI-based approach better suited for server administration.
